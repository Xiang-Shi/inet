...
TCPEventCode TCPConnection::processSegment1stThru8th(TCPSegment *tcpseg)
{
        ...
        fsm.getState() == TCP_S_FIN_WAIT_1 || fsm.getState() == TCP_S_FIN_WAIT_2)
    {
        ...
        if (tcpseg->getPayloadLength() > 0) {
            ...
            EV_INFO<<"Receiving Segment Before Truncate:["<<tcpseg->getSequenceNo()<<", "<<tcpseg->getSequenceNo()+tcpseg->getPayloadLength()<<"),"<<endl;
            EV_INFO<<"rcv_nxt ="<<state->rcv_nxt<<" , rcv_nxt_seq ="<<state->rcv_nxt_seq<<" , rcv_wnd ="<<state->rcv_wnd<<endl;


            //make sure not to receive overlapped region. making (tcpsequenceNo >= state->rcv_nxt)
            ...
            if (hasEnoughSpaceForSegmentInReceiveQueue(tcpseg)) {    // enough freeRcvBuffer in rcvQueue for new segment?
                ...
                if (rcvSegVector)
                    rcvSegVector->record(state->rcv_seg);

                if(payloadVector)
                    payloadVector->record(tcpseg->getPayloadLength());


                //SHI:  Before moving into the receive queue,
                //read the timestamp of EACH TCP segment,
                //and calculate average packet delay time(ignore the queueing time in rcvQueue)
                if (state->ts_enabled && pkDelayVector) {
                    uint32_t testTS = getTSval(tcpseg);

                    simtime_t tSent = convertTSToSimtime(testTS);
                    simtime_t pkDelay = simTime() - tSent;

                    pkDelayVector->record(pkDelay);
                }


                //SHI: (RD:Reordering Density) record the distribution of the displacement of EACH arrived TCP segments
                int rcv_seg_tmp = state->rcv_seg;
                int sendSeqNo_tmp = tcpseg->getSendSeqNo();

                int disorder = 0;
                disorder = rcv_seg_tmp - sendSeqNo_tmp;

                if (disorderVector)
                    disorderVector->record(disorder);


                //-------------------------RBD start-------------------------------------


                //SHI: if rcv_nxt_seq changes, we need calculate the difference between them to calculate RBD
               uint32 old_rcv_nxt_seq = state->rcv_nxt_seq;

               if(rcvNxtSeqVector)
                    rcvNxtSeqVector->record(state->rcv_nxt_seq);

               EV_INFO<<"Receiving Segment After Truncate:["<<tcpseg->getSequenceNo()<<", "<<tcpseg->getSequenceNo()+tcpseg->getPayloadLength()<<"),"<<endl;
               EV_INFO<<"rcv_nxt ="<<state->rcv_nxt<<" , rcv_nxt_seq ="<<state->rcv_nxt_seq<<" , rcv_wnd ="<<state->rcv_wnd<<endl;


               ASSERT(seqGE(tcpseg->getSequenceNo(), state->rcv_nxt));
               ASSERT(seqGE(tcpseg->getSendSeqNo(), state->rcv_nxt_seq));
               state->rcv_nxt_seq = rcvSeqQueue->enqueueRcvSegment(tcpseg); //record received segments (non-duplicate segments, and with data)


                if(tcpseg->getSendSeqNo()  >= old_rcv_nxt_seq)
                {
                    if(tcpseg->getSendSeqNo() == old_rcv_nxt_seq)
                    {
                        //Expected segment has arrived
                        old_rcv_nxt_seq++;
                        while(seqLess(old_rcv_nxt_seq,state->rcv_nxt_seq))
                        {
                            state->rcv_buf_occup--;
                            old_rcv_nxt_seq++;
                        }
                ...
                if(rcvBufOccupVector)
                    rcvBufOccupVector->record(state->rcv_buf_occup);
                ...
                if (old_rcv_nxt == state->rcv_nxt) {
                    ...
                    // record the number of out-of-order segment
                    if (rcvOooSegVector)
...
TCPEventCode TCPConnection::processSegmentInSynSent(TCPSegment *tcpseg, L3Address srcAddr, L3Address destAddr)
{
    ...
    if (tcpseg->getSynBit()) {
        ...
        if (tcpseg->getAckBit()) {
            ...
            sentSeqQueue->discardUpTo(state->snd_una); //SHI
...
TCPEventCode TCPConnection::processRstInSynReceived(TCPSegment *tcpseg)
{
    ...
    sentSeqQueue->discardUpTo(sentSeqQueue->getBufferEndSeq()); //SHI:　flush sent sequence queue
...
bool TCPConnection::processAckInEstabEtc(TCPSegment *tcpseg)
{
    ...
    if (seqGE(state->snd_una, tcpseg->getAckNo())) {
        ...
        //SHI: acked data no longer needed in sentSeqQueue
        sentSeqQueue->discardUpTo(discardUpToSeq);
