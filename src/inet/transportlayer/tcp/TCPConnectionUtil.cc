...
void TCPConnection::initConnection(TCPOpenCommand *openCmd)
{
    ...
    // SHI: create sentSeqQueue
    sentSeqQueue = new TCPSentSeqQueue();
    sentSeqQueue->setConnection(this);

    // SHI: create rcvSeqQueue
    rcvSeqQueue = new TCPRcvSeqQueue();
    rcvSeqQueue->setConnection(this);
    ...
    if (tcpseg->getPayloadLength() > 0)
    {
        if (seqLess(tcpseg->getSequenceNo(), state->snd_max))
        {
          ...
          // tcpseg->setSendSeqNo(sentseq);
           ...
           if(resentSegVector)
               resentSegVector->record(tcpseg->getSequenceNo());
        ...
        else
        {
            ...
            //SHI: first-sent segment: record send sequence
            //send sequence start from 1
            state->snd_seg++;
            tcpseg->setSendSeqNo(state->snd_seg);

            EV_DETAIL << "First Sent Segment, the "<<tcpseg->getSendSeqNo()<<"th:["
                    << tcpseg->getSequenceNo() << "," << tcpseg->getSequenceNo() + tcpseg->getPayloadLength()<<")"<<endl;
            sentSeqQueue->enqueueSentDataSeq(tcpseg->getSequenceNo(), tcpseg->getSequenceNo() + tcpseg->getPayloadLength(), tcpseg->getSendSeqNo());

            if (sndSegVector)
                sndSegVector->record(state->snd_seg);
        }
...
//    EV_INFO << "SendData(): calculating effectiveWin, state->snd_wnd = " << state->snd_wnd
//            << " , congestionWindow = " << congestionWindow
//            << " ,  state->snd_nxt = " << state->snd_nxt
//            << " ,  state->snd_una = " << state->snd_una << endl;
...
//    EV_INFO << "retransmitOneSegment(): calculating send bytes, state->snd_mss = " << state->snd_mss
//              << " , state->snd_max = " << state->snd_max
//              << " ,  state->snd_nxt = " << state->snd_nxt
//              << " ,  sendQueue bytes available = " << sendQueue->getBytesAvailable(state->snd_nxt) << endl;
