...
class TCPSegment;
class TCPSendQueue;
class TCPSentSeqQueue;
class TCPRcvSeqQueue;
...
class INET_API TCPStateVariables : public cObject
{
  ...
  public:
    ...
    uint32 snd_seg;     //SHI: record the number of sent segments
    uint32 retransmission_num;  //SHI: record the retransmission times
    ...
    uint32 rcv_buf_occup; //SHI: record the receiver buffer occupancy, to calculate its density
    ...
    //SHI: counters of received segment number, for calculating RD(reordering density)
    uint32 rcv_seg;
    //SHI: receive nxt send sequence; expected segment number (expected send sequence of segment)
    uint32 rcv_nxt_seq;
    //SHI: counters of fast retransmit/recovery times
    uint32 fastR_num;
...
class INET_API TCPConnection
{
  ...
  public:
    ...
    // SHI: TCP sent queue recording sent sequence
     TCPSentSeqQueue *sentSeqQueue = nullptr;

    // SHI: TCP receive queue recording sent sequence
     TCPRcvSeqQueue *rcvSeqQueue = nullptr;
  ...
  protected:
    ...
    // statistics
    //SHI: record  packet delay
    cOutVector *pkDelayVector = nullptr;

    //SHI: record disorder data of received TCP segments
    //(Reordering Density: the discrete density of the frequency of packets with respect to their displacements)
    cOutVector *disorderVector = nullptr;

    //SHI: record the tcp segment payload
    cOutVector *payloadVector = nullptr;

    //SHI:  recorder for testing
    cOutVector *test_vector = nullptr;

    cOutVector *sndSegVector = nullptr;     //SHI:number of sent segments
    cOutVector *resentSegVector = nullptr; //SHI: record the information of resent segments
    cOutVector *retransmissionVector = nullptr; //SHI: record the time of retransmission(both RTO and fast retransmission)
    ...
    cOutVector *rcvSegVector = nullptr;     //SHI:number of received segments
    cOutVector *rcvNxtSeqVector = nullptr;     //SHI:  send sequence of expected Segment
    cOutVector *rcvBufOccupVector = nullptr; //SHI: value of receive buffer occupancy
  ...
  public:
    ...
    TCPSentSeqQueue *getSentSeqQueue() { return sentSeqQueue; }
    TCPRcvSeqQueue *getRcvSeqQueue() { return rcvSeqQueue; }
