...
void TCPSegment::truncateSegment(uint32 firstSeqNo, uint32 endSeqNo)
{
    ...
    //the two area must have common part: Area1(firstSeqNo, endSeqNo), Area2(sequenceNo_var, sequenceNo_var + payloadLength_var)
#ifndef NDEBUG
    ...
    //if the Area2 is on the left of Area1
...
void TCPSegment::truncateData(unsigned int truncleft, unsigned int truncright)
{
    ...
    //used only when dataTransferMode == TCP_TRANSFER_BYTESTREAM
    if (0 != byteArray_var.getDataArraySize())
        byteArray_var.truncateData(truncleft, truncright);

    //clean the payloadList when (it's not empty and it's first element is smaller than truncleft in size)
...
//return msg and its corresponding SequenceNo
cPacket *TCPSegment::removeFirstPayloadMessage(uint32& endSequenceNo)
{
    ...
    payloadList.pop_front();
    drop(msg); //change the msg owner from TCPSegment to TCP
    return msg;
}
